...
export default {
  ...
  actions: {
    ...
    addItem: async ({ commit }, { title, cost, volume, receptionist }) => {
      const item = await api.add({ title, cost, volume, receptionist })
      commit('setItem', item)
    },
    updateItem: async ({ commit }, { id, title, cost, volume, receptionist }) => {
      const item = await api.update({ id, title, cost, volume, receptionist });

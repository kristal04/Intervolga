...
export default {
  ...
  actions: {
    fetchItems: async ({ commit }) => {
      const response = await api.receptionists();
    ...
    addItem: async ({ commit }, { receptionist, name }) => {
      const item = await api.add({ receptionist, name });
      commit('addItem', item);
    },
    updateItem: async ({ commit }, { id, receptionist, name }) => {
      const item = await api.update({ id, receptionist, name });
